...
export class NavContentComponent implements OnInit, AfterViewInit {
  ...
  constructor(private _local: LocalService, public nav: NavigationItem, private zone: NgZone, private location: Location) {
    ...
    for(let i = 0; i<this.navigation.length; i++){
      for(let j = 0; j<this.navigation[i].children.length; j++){
        for(let k = 0; k<this.permisos.length; k++){
          this.navigation[i].children[j].hidden = false;
          if(this.permisos[k] == this.navigation[i].children[j].id){
            this.navigation[i].children[j].hidden = false;
            break;
          }else{
            this.navigation[i].children[j].hidden = true;
          }
        }
      }
    }
